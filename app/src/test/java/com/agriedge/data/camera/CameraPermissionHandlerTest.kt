...
class CameraPermissionHandlerTest {
    ...
    fun testHasPermission() {
        every { context.checkSelfPermission(Manifest.permission.CAMERA) } returns PackageManager.PERMISSION_GRANTED
    ...
    fun testHasNoPermission() {
        every { context.checkSelfPermission(Manifest.permission.CAMERA) } returns PackageManager.PERMISSION_DENIED
        val hasPermission = CameraPermissionHandler.hasPermission(context)
    ...
    fun testPermissionDeniedCallback() {
        val onDenied: (Boolean) -> Unit = { shouldShowRationale = it; permissionDeniedCalled = true }
        onDenied(true)
